...
static void one_3d_dp_init (void);
static void one_3d_dp_deinit (void);
static void one_3d_dp_update (uint64_t tickCounter);
static void one_3d_dp_render (void);

//-----
void one_3d_dp_register_callbacks (DP_ObjFcns_t *objFcns)
{
    objFcns->dp_init = one_3d_dp_init;
    objFcns->dp_deinit = one_3d_dp_deinit;
    objFcns->dp_update = one_3d_dp_update;
    objFcns->dp_render = one_3d_dp_render;
}
...
static void one_3d_dp_init (void)
{
    ...
    for (int i = 0; i < gThis.noOfSystems; i++)
    {
        ...
        syst->anchorPos = (Vector3) {0.0f, 0.0f, 1.0f + i};
...
static void one_3d_dp_deinit (void)
{
    LOGI ("one_3d_dp_deinit");

    UnloadModel (gThis.model);
}

//-----
static void one_3d_dp_update (uint64_t tc)
{
//    LOGI ("one_3d_dp_update tc %lu", gThis.tickCounter);
